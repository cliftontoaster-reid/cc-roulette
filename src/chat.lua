	...
	if event.message == "register" then
		if tmpBet.player ~= nil then
			...
			elseif tmpBet.player ~= event.username then
				msg.sendMessageToPlayer(
					"A player is currently registered, please wait for them to place a bet",
					event.username
				)
			end
			return
...
local function handleCoin(event)
	...
	if tmpBet.player == nil then
		msg.sendMessage(
			"No player registered, please use '$register' before placing a bet, there are currently "
				.. tmpBet.amount
	...
	msg.sendMessageToPlayer(
		"Coin received, a total of " .. tmpBet.amount .. " coins are now in your account",
		tmpBet.player
	)
	return 0
end
...
local function getBet()
	if tmpBet.player == nil then
		msg.sendMessage(
			"No player is currently registered, please use '$register' before placing a bet, there are "
				.. tmpBet.amount
				.. " coins without an owner"
		)
		return nil
